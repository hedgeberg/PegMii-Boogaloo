    ...
    try:
        if cmd == "help":
            ...
            for thread in threads:
                table.append([thread.name,thread.core,'%08X' % thread.stackBase,'%08X' % thread.stackEnd,'%08X' % thread.entryPoint,thread.priority])
            print(tabulate.tabulate(table,["Name","Core","Stack Start","Stack End","Entrypoint","Priority"]))
